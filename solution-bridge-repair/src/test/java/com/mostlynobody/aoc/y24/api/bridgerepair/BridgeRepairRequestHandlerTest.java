package com.mostlynobody.aoc.y24.api.bridgerepair;

import com.amazonaws.services.lambda.runtime.events.APIGatewayProxyRequestEvent;
import com.amazonaws.services.lambda.runtime.events.APIGatewayProxyResponseEvent;
import com.mostlynobody.aoc.y24.service.bridgerepair.BridgeRepairService;
...
public class BridgeRepairRequestHandlerTest {

    private final JsonMapper objectMapper = mock(JsonMapper.class);
    private final BridgeRepairService bridgeRepairService = mock(BridgeRepairService.class);

    private final BridgeRepairRequestHandler functionRequestHandler =
            new BridgeRepairRequestHandler(objectMapper, bridgeRepairService);
    ...
    void testExecute_Success() throws IOException {
        ...
        when(bridgeRepairService.solve("testInput")).thenReturn(solutionJson);
        ...
        verify(bridgeRepairService).solve("testInput");
        verify(objectMapper).writeValueAsString(solutionJson);
        verifyNoMoreInteractions(objectMapper, bridgeRepairService);
    }
    ...
    void testExecute_MissingRequestBody() {
        ...
        verifyNoInteractions(objectMapper, bridgeRepairService);
    }
    ...
    void testExecute_MalformedInputJson() throws IOException {
        ...
        verify(bridgeRepairService, never()).solve(anyString());
    ...
    void testExecute_ServiceThrowsException() throws IOException {
        ...
        when(objectMapper.readValue(inputJsonString, InputJson.class)).thenReturn(inputJson);
        when(bridgeRepairService.solve("testInput")).thenThrow(new RuntimeException("Service Failed"));
