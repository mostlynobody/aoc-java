...
public class CeresSearchRequestHandlerTest {

    private final JsonMapper objectMapper = mock(JsonMapper.class);
    private final CeresSearchService ceresSearchService = mock(CeresSearchService.class);

    private final CeresSearchRequestHandler functionRequestHandler =
            new CeresSearchRequestHandler(objectMapper, ceresSearchService);
    ...
    void testExecute_Success() throws IOException {
        ...
        when(ceresSearchService.solve("testInput")).thenReturn(solutionJson);
        ...
        verify(ceresSearchService).solve("testInput");
        verify(objectMapper).writeValueAsString(solutionJson);
        verifyNoMoreInteractions(objectMapper, ceresSearchService);
    }
    ...
    void testExecute_MissingRequestBody() {
        ...
        verifyNoInteractions(objectMapper, ceresSearchService);
    }
    ...
    void testExecute_MalformedInputJson() throws IOException {
        ...
        verify(ceresSearchService, never()).solve(anyString());
    ...
    void testExecute_ServiceThrowsException() throws IOException {
        ...
        when(objectMapper.readValue(inputJsonString, InputJson.class)).thenReturn(inputJson);
        when(ceresSearchService.solve("testInput")).thenThrow(new RuntimeException("Service Failed"));
