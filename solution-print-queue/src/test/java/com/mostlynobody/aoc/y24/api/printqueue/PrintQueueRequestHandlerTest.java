...
public class PrintQueueRequestHandlerTest {

    private final JsonMapper objectMapper = mock(JsonMapper.class);
    private final PrintQueueService printQueueService = mock(PrintQueueService.class);

    private final PrintQueueRequestHandler functionRequestHandler =
            new PrintQueueRequestHandler(objectMapper, printQueueService);
    ...
    void testExecute_Success() throws IOException {
        ...
        when(printQueueService.solve("testInput")).thenReturn(solutionJson);
        ...
        verify(printQueueService).solve("testInput");
        verify(objectMapper).writeValueAsString(solutionJson);
        verifyNoMoreInteractions(objectMapper, printQueueService);
    }
    ...
    void testExecute_MissingRequestBody() {
        ...
        verifyNoInteractions(objectMapper, printQueueService);
    }
    ...
    void testExecute_MalformedInputJson() throws IOException {
        ...
        verify(printQueueService, never()).solve(anyString());
    ...
    void testExecute_ServiceThrowsException() throws IOException {
        ...
        when(objectMapper.readValue(inputJsonString, InputJson.class)).thenReturn(inputJson);
        when(printQueueService.solve("testInput")).thenThrow(new RuntimeException("Service Failed"));
