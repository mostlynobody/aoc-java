...
public class DiskFragmenterRequestHandlerTest {

    private final JsonMapper objectMapper = mock(JsonMapper.class);
    private final DiskFragmenterService diskFragmenterService = mock(DiskFragmenterService.class);

    private final DiskFragmenterRequestHandler functionRequestHandler =
            new DiskFragmenterRequestHandler(objectMapper, diskFragmenterService);
    ...
    void testExecute_Success() throws IOException {
        ...
        when(diskFragmenterService.solve("testInput")).thenReturn(solutionJson);
        ...
        verify(diskFragmenterService).solve("testInput");
        verify(objectMapper).writeValueAsString(solutionJson);
        verifyNoMoreInteractions(objectMapper, diskFragmenterService);
    }
    ...
    void testExecute_MissingRequestBody() {
        ...
        verifyNoInteractions(objectMapper, diskFragmenterService);
    }
    ...
    void testExecute_MalformedInputJson() throws IOException {
        ...
        verify(diskFragmenterService, never()).solve(anyString());
    ...
    void testExecute_ServiceThrowsException() throws IOException {
        ...
        when(objectMapper.readValue(inputJsonString, InputJson.class)).thenReturn(inputJson);
        when(diskFragmenterService.solve("testInput")).thenThrow(new RuntimeException("Service Failed"));
