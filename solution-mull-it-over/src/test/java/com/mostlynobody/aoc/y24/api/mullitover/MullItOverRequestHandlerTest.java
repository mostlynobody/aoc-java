...
public class MullItOverRequestHandlerTest {

    private final JsonMapper objectMapper = mock(JsonMapper.class);
    private final MullItOverService mullItOverService = mock(MullItOverService.class);

    private final MullItOverRequestHandler functionRequestHandler =
            new MullItOverRequestHandler(objectMapper, mullItOverService);
    ...
    void testExecute_Success() throws IOException {
        ...
        when(mullItOverService.solve("testInput")).thenReturn(solutionJson);
        ...
        verify(mullItOverService).solve("testInput");
        verify(objectMapper).writeValueAsString(solutionJson);
        verifyNoMoreInteractions(objectMapper, mullItOverService);
    }
    ...
    void testExecute_MissingRequestBody() {
        ...
        verifyNoInteractions(objectMapper, mullItOverService);
    }
    ...
    void testExecute_MalformedInputJson() throws IOException {
        ...
        verify(mullItOverService, never()).solve(anyString());
    ...
    void testExecute_ServiceThrowsException() throws IOException {
        ...
        when(objectMapper.readValue(inputJsonString, InputJson.class)).thenReturn(inputJson);
        when(mullItOverService.solve("testInput")).thenThrow(new RuntimeException("Service Failed"));
