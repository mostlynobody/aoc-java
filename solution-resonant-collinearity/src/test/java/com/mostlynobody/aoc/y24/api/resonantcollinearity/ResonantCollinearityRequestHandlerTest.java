...
public class ResonantCollinearityRequestHandlerTest {

    private final JsonMapper objectMapper = mock(JsonMapper.class);
    private final ResonantCollinearityService resonantCollinearityService = mock(ResonantCollinearityService.class);

    private final ResonantCollinearityRequestHandler functionRequestHandler =
            new ResonantCollinearityRequestHandler(objectMapper, resonantCollinearityService);
    ...
    void testExecute_Success() throws IOException {
        ...
        when(resonantCollinearityService.solve("testInput")).thenReturn(solutionJson);
        ...
        verify(resonantCollinearityService).solve("testInput");
        verify(objectMapper).writeValueAsString(solutionJson);
        verifyNoMoreInteractions(objectMapper, resonantCollinearityService);
    }
    ...
    void testExecute_MissingRequestBody() {
        ...
        verifyNoInteractions(objectMapper, resonantCollinearityService);
    }
    ...
    void testExecute_MalformedInputJson() throws IOException {
        ...
        verify(resonantCollinearityService, never()).solve(anyString());
    ...
    void testExecute_ServiceThrowsException() throws IOException {
        ...
        when(objectMapper.readValue(inputJsonString, InputJson.class)).thenReturn(inputJson);
        when(resonantCollinearityService.solve("testInput")).thenThrow(new RuntimeException("Service Failed"));
