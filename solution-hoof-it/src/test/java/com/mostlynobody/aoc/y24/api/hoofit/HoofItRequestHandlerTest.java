...
public class HoofItRequestHandlerTest {

    private final JsonMapper objectMapper = mock(JsonMapper.class);
    private final HoofItService hoofItService = mock(HoofItService.class);

    private final HoofItRequestHandler functionRequestHandler =
            new HoofItRequestHandler(objectMapper, hoofItService);
    ...
    void testExecute_Success() throws IOException {
        ...
        when(hoofItService.solve("testInput")).thenReturn(solutionJson);
        ...
        verify(hoofItService).solve("testInput");
        verify(objectMapper).writeValueAsString(solutionJson);
        verifyNoMoreInteractions(objectMapper, hoofItService);
    }
    ...
    void testExecute_MissingRequestBody() {
        ...
        verifyNoInteractions(objectMapper, hoofItService);
    }
    ...
    void testExecute_MalformedInputJson() throws IOException {
        ...
        verify(hoofItService, never()).solve(anyString());
    ...
    void testExecute_ServiceThrowsException() throws IOException {
        ...
        when(objectMapper.readValue(inputJsonString, InputJson.class)).thenReturn(inputJson);
        when(hoofItService.solve("testInput")).thenThrow(new RuntimeException("Service Failed"));
